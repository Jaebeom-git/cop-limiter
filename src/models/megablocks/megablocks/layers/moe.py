...
# def batched_load_balancing_loss(args: Arguments):
#     """ 
#     기존처럼 모아서 최종 loss를 만드는 함수.
#     여기서는 _LOAD_BALANCING_LOSS에 쌓인 balance_loss들을 평균 내거나 합산.
#     """
#     if args.moe_loss_weight == 0:
#         return 0.0

#     balance_losses = get_load_balancing_loss()
#     if len(balance_losses) == 0:
#         return 0.0

#     # 예: 여러 번 호출된 balance_loss들의 평균을 구하고,
#     #     moe_loss_weight를 곱해서 최종 loss로 삼는다.
#     #     (정책에 따라 합(sum)으로 할 수도 있음)
#     stacked = torch.stack(balance_losses)
#     final_loss = stacked.sum() * args.moe_loss_weight
...
def _save_router_logits(logits: torch.Tensor):
...
class ParallelMLP(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, scores: torch.Tensor, expert_weights: torch.Tensor, top_experts: torch.Tensor):
        ...
        if self.args.moe_loss_weight > 0:
            ...
            # save_load_balancing_loss(balance_loss)
        x = x.view(in_shape)
...
class MoE(torch.nn.Module):
    def __init__(self, args: Arguments):
        ...
        ## SparseMixer
        self.gating_network = torch.nn.Linear(
            args.hidden_size,
        ...
        args.init_method(self.gating_network.weight)
        self.gating_dropout = torch.nn.Dropout(p=args.moe_dropout)
        
        # compute_balance_loss = True if args.moe_loss_weight > 0 else False
        compute_balance_loss = False
        self.router = SparseMixerV2(num_experts=args.moe_num_experts, embed_dim=args.hidden_size, compute_balance_loss=compute_balance_loss, jitter_eps=args.moe_jitter_eps, top_k=args.moe_top_k).to(args.device)

        # Expert computation helper.
        self.experts = self._init_experts_mlp(args)
        self.expert_dropout = torch.nn.Dropout(p=args.moe_dropout)

        self.shared_expert = None
        if args.shared_expert:
            # SharedExpert computation helper.
            self.shared_expert = sharedexpert_registry.get(args)
            self.shared_expert_dropout = torch.nn.Dropout(p=args.moe_dropout)
    ...
    def forward(self, x: torch.Tensor):
        ...
        with torch.amp.autocast(enabled=True, dtype=torch.float32, device_type="cuda"):
            logits = self.gating_network(x.view(-1, x.shape[-1]))
            _save_router_logits(logits)
            logits = self.gating_dropout(logits)

            # sample, multiplier, balance_loss = self.router(logits)
            # out = self.experts(x, balance_loss, multiplier, sample)
            sample, multiplier, scores = self.router(logits)

        out = self.experts(x, scores, multiplier, sample)
        out = self.expert_dropout(out)

        # Compute the experts.
        # out = self.experts(x, scores, expert_weights, top_experts)
        if self.shared_expert is not None:
            shared_expert_out = self.shared_expert(x)
            shared_expert_out = self.shared_expert_dropout(shared_expert_out)
